...
func _on_state_changed(from, to):
	...
	state_machine.get_state(from).state_impl.exit(get_parent(), to)
	state_machine.get_state(to).state_impl.enter(get_parent(), from)
...
func update(delta=get_physics_process_delta_time()):
	...
	state_machine.get_state(current_state).state_impl.update(get_parent(), delta)
	if update_process_mode == UpdateProcessMode.MANUAL:
